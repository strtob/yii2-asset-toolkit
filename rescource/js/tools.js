...
$(document).on('click', '.showModal', function (event) {
    ...
    // Modal IDs for original and copy
    var original = invoker.data('bs-target');
    var copy = invoker.data('bs-target-copy');
    var makeCopy = (copy !== undefined);
    var elModal = $(original);

    // Handle modal copying if necessary
    if (makeCopy) {
        console.log('Copy element ' + original + ' to element ' + copy);
        elModal = $(copyElement(original, copy));
    }
